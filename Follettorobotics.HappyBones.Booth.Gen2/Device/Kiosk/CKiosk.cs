...
{
    public class CKiosk : SocketServer
    {
                ...
                if (Array.Exists(msg, match => match.Equals(CONST.S_CRLF)))
                {
                    switch (msg[0].ToUpper().Trim())
                    {
                        ...
                        case "STATUS":
                            ...
                            GlobalDevice.Kiosk.Instance?.Send(clientIndex, $"Status|{GlobalVariable.Parameter[(int)e_Parameter.Booth][CONST.S_KEY][(int)e_Parameter_Booth.Booth_No]}|{status}{CONST.S_CRLF}", call);
                            break;
                        case "WAITING":
                            ...
                            GlobalDevice.Kiosk.Instance?.Send(clientIndex, $"Waiting|{총대기건수}/{대기예상시간}|{CONST.S_CRLF}", call);
                            break;
                        case "ORDER":
                            ...
                            for (int i = 5; i < Array.IndexOf(msg, CONST.S_CRLF); i++)
                            {
                                ...
                                if (orders.Length == 4)
                                {
                                    cmdText = $" INSERT INTO {e_DB._Order}({e_DB_Order.Column00},{e_DB_Order.Column01},{e_DB_Order.Column02},{e_DB_Order.Column04},{e_DB_Order.Column05},{e_DB_Order.Column06},{e_DB_Order.Column07},{e_DB_Order.Column08},{e_DB_Order.Column09},{e_DB_Order.Column10},{e_DB_Order.Column12})" +
                                              $" VALUES('{DateTime}','{$"{ID}_{i - 4:000}"}','{e_Order_Status.주문}','{e_Order_Source.KIOSK}','{msg[2/*오더번호*/]}','{msg[3/*QR코드Data*/]}','{orders[0/*제품코드*/].ToUpper()}','{orders[1/*제품명*/]}','{orders[2/*결제구분*/]}','{orders[3/*상품금액*/]}','{e_Order_Cup.TAKEOUT}')";
                                }
                                else if (orders.Length == 5)
                                {
                                    ...
                                    try
                                    {
                                        Order_Cup = (e_Order_Cup)Enum.Parse(typeof(e_Order_Cup), orders[4/*Order_Cup*/]);
                                    }
                                    catch
                                    {
                                    }
                                    ...
                                    cmdText = $" INSERT INTO {e_DB._Order}({e_DB_Order.Column00},{e_DB_Order.Column01},{e_DB_Order.Column02},{e_DB_Order.Column04},{e_DB_Order.Column05},{e_DB_Order.Column06},{e_DB_Order.Column07},{e_DB_Order.Column08},{e_DB_Order.Column09},{e_DB_Order.Column10},{e_DB_Order.Column12})" +
                                              $" VALUES('{DateTime}','{$"{ID}_{i - 4:000}"}','{e_Order_Status.주문}','{e_Order_Source.KIOSK}','{msg[2/*오더번호*/]}','{msg[3/*QR코드Data*/]}','{orders[0/*제품코드*/].ToUpper()}','{orders[1/*제품명*/]}','{orders[2/*결제구분*/]}','{orders[3/*상품금액*/]}','{Order_Cup}')";
                                }
                                else
                                {
                            ...
                            if (order)
                            {
                                GlobalDevice.Kiosk.Instance?.Send(clientIndex, $"Order|{msg[2/*오더번호*/]}|{CONST.S_CRLF}", call);
                            }
                            break;
                        case "CONNECT":
                            GlobalDevice.Kiosk.Instance?.Send(clientIndex, $"Connect|{GlobalVariable.Parameter[(int)e_Parameter.Booth][CONST.S_KEY][(int)e_Parameter_Booth.Booth_No]}|{CONST.S_CRLF}", call);
                            break;
